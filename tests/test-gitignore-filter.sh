#!/bin/bash
# Test suite for ib test-gitignore-filter command
# Run from repo root: ./tests/test-gitignore-filter.sh
#
# Fixture naming convention: {expected-result}-{description}.json
# The expected result (pass or fail) is extracted from the filename prefix (before first hyphen).
# pass = filter produces expected output
# fail = filter does not produce expected output (used to verify test harness works)
...
FIXTURES_DIR="$SCRIPT_DIR/fixtures/gitignore-filter"
...
echo "Running test-gitignore-filter tests..."
echo "========================================"
echo ""
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    # Run ib test-gitignore-filter with fixture file
    actual=$("$IB" test-gitignore-filter "$fixture_path" 2>&1) || true

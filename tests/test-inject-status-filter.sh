#!/bin/bash
# Test suite for ib test-inject-status-filter command
# Run from repo root: ./tests/test-inject-status-filter.sh
#
# Fixture naming convention: {expected-result}-{description}.json
# The expected result (skip or inject) is extracted from the filename prefix (before first hyphen).
...
FIXTURES_DIR="$SCRIPT_DIR/fixtures/inject-status-filter"
...
echo "Running test-inject-status-filter tests..."
echo "========================================"
echo ""
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    # Run ib test-inject-status-filter with fixture file
    actual=$("$IB" test-inject-status-filter "$fixture_path" 2>&1) || true

#!/bin/bash
# Test suite for ib test-main-path command
# Run from repo root: ./tests/test-main-path.sh
#
...
echo "Running test-main-path tests..."
echo "========================================"
echo ""
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    # Run ib test-main-path with fixture file
    actual=$("$IB" test-main-path "$fixture_path" 2>&1) || true

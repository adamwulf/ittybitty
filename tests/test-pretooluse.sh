#!/bin/bash
# Test suite for ib test-pretooluse command (PreToolUse hook logic)
# Run from repo root: ./tests/test-pretooluse.sh
#
# Fixture naming convention: {expected-decision}-{description}.json
# The expected decision is extracted from the filename prefix (before first hyphen).
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    description=$(jq -r '.description // ""' "$fixture_path" 2>/dev/null)
    if [[ -z "$description" ]]; then
        # Fallback: extract from filename (everything after first hyphen, minus .json)
        description="${filename#*-}"
        description="${description%.json}"
        description="${description//-/ }"
    fi

    # Run ib test-pretooluse with file argument

#!/bin/bash
# Test suite for ib test-setup-check command
# Run from repo root: ./tests/test-setup-check.sh
#
# Fixture naming convention: {expected-result}-{check-type}-{description}.json
# The expected result (1 or 0) is extracted from the filename prefix (before first hyphen).
# 1 = checkbox should be checked (pattern found)
# 0 = checkbox should be unchecked (pattern not found)
...
FIXTURES_DIR="$SCRIPT_DIR/fixtures/setup-check"
...
echo "Running test-setup-check tests..."
echo "========================================"
echo ""
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    # Run ib test-setup-check with fixture file
    actual=$("$IB" test-setup-check "$fixture_path" 2>&1) || true

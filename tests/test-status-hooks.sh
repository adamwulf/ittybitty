#!/bin/bash
# Test suite for status hooks JSON helper functions
# Run from repo root: ./tests/test-status-hooks.sh
#
# Tests json_has_status_hooks, json_add_status_hooks, json_remove_status_hooks
...
FIXTURES_DIR="$SCRIPT_DIR/fixtures/status-hooks"
...
echo "Running test-status-hooks tests..."
echo "========================================"
echo ""
...
for fixture_path in "$FIXTURES_DIR"/*.json; do
    ...
    # Run ib test-status-hooks with fixture file
    actual=$("$IB" test-status-hooks "$fixture_path" 2>&1) || true
